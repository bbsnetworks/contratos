...
const btnGenerarContratoBottom = document.getElementById('btnGenerarContratoBottom');
...
function cargarVistaContratoFibra() {
  ...
  visor.innerHTML = imagenes.map((nombre, index) => `
...
function initSignaturePads() {
  ...
  resizeCanvasPreserve(signatureCanvas, signaturePadPreview, signatureImageSaved1);
}
...
function openSignatureModalFibra() {
  ...
  setTimeout(() => {
    resizeCanvasPreserve(signatureCanvasModal, signaturePadModal, signatureImageSaved1);
...
function bindSignatureEvents() {
  ...
  saveSignatureModal?.addEventListener('click', saveModalSignatureToPreviewFibra);
  ...
  window.addEventListener('resize', () => {
    ...
    if (signatureModal && !signatureModal.classList.contains('hidden') && signaturePadModal) {
...
function validarFormulario() {
  ...
  const firmaExiste =
  !!signatureImageSaved1 || (signaturePadPreview && !signaturePadPreview.isEmpty());
...
function normalizarDatosFibra(datos) {
  return {
    ...
    metodoPago: {
      ...
      transferencia: !!datos.metodoPago.transferenciaBancaria,
      deposito: !!datos.metodoPago.depositoCuenta,
      tiendasServicios: !!datos.metodoPago.pagoTiendasServicios,
      tarjeta: !!datos.metodoPago.tarjetaCreditoDebito,
      domiciliado: !!datos.metodoPago.domiciliadoTarjeta,
      enLinea: !!datos.metodoPago.pagoEnLinea,
      centrosServicio: !!datos.metodoPago.pagoTiendasCentrosServicio,
...
function obtenerDatosFibra() {
  return {
    ...
    metodoPago: {
      efectivo: checked('mpEfectivo'),
      transferenciaBancaria: checked('mpTransferencia'),
      depositoCuenta: checked('mpDeposito'),
      pagoTiendasServicios: checked('mpTiendasServicios'),
      tarjetaCreditoDebito: checked('mpTarjeta'),
      domiciliadoTarjeta: checked('mpDomiciliado'),
      pagoEnLinea: checked('mpEnLinea'),
      pagoTiendasCentrosServicio: checked('mpCentrosServicio'),
    ...
    firmaCliente: signatureImageSaved1 ||
...
function accionGenerar() {
  ...
  Swal.fire({
    ...
    text: 'La captura base del contrato de fibra está correcta. El siguiente paso es conectarlo con el PDF.',
    background: '#0b1120',
    color: '#e2e8f0',
    confirmButtonColor: '#0284c7'
  });
...
const firmaCliente = datosFormulario.firmaCliente || null;
  ...
  pdf.addImage(
    ...
    if (firmaCliente) {
  pdf.addImage(firmaCliente, "PNG", mapaFibra.p2.firmaProveedor.x, mapaFibra.p2.firmaProveedor.y, mapaFibra.p2.firmaProveedor.w, mapaFibra.p2.firmaProveedor.h);
  pdf.addImage(firmaCliente, "PNG", mapaFibra.p2.firmaProvedor2.x, mapaFibra.p2.firmaProvedor2.y, mapaFibra.p2.firmaProvedor2.w, mapaFibra.p2.firmaProvedor2.h);
  pdf.addImage(firmaCliente, "PNG", mapaFibra.p2.firmaProvedor3.x, mapaFibra.p2.firmaProvedor3.y, mapaFibra.p2.firmaProvedor3.w, mapaFibra.p2.firmaProvedor3.h);
  pdf.addImage(firmaCliente, "PNG", mapaFibra.p2.firmaProvedor4.x, mapaFibra.p2.firmaProvedor4.y, mapaFibra.p2.firmaProvedor4.w, mapaFibra.p2.firmaProvedor4.h);
}
    // Página 3
    pdf.addPage();
